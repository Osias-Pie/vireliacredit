...
import { answerFromVireliaKnowledge, type AssistantKnowledgeContext } from "@/lib/assistant-knowledge";

export interface AssistantPageContext extends AssistantKnowledgeContext {}
...
const VISIBLE_ON = ["/", "/programs", "/eligibility", "/apply", "/suivi", "/confirmation", "/faq", "/about", "/contact"];
...
export function VireliaAssistant() {
  ...
  const visible = VISIBLE_ON.some((p) => (p === "/" ? pathname === "/" : pathname === p || pathname.startsWith(`${p}/`)));
  if (!visible) return null;

  async function send() {
    const text = input.trim();
    if (!text || pending) return;
    const context = bag?.context ?? { page: pathname.slice(1) || "home" };
    setInput("");
  ...
  return (
    <div className="fixed bottom-4 right-4 z-50 flex flex-col items-end gap-2 sm:bottom-6 sm:right-6">
      {open && (
        <div className="flex h-[min(28rem,70vh)] w-[min(22rem,calc(100vw-2rem))] flex-col overflow-hidden rounded-2xl border border-gold/30 bg-background shadow-xl">
          ...
          <ScrollArea className="flex-1 px-4 py-3">
            {messages.length === 0 && <p className="text-sm text-muted-foreground">{t("assistant.empty")}</p>}
            <div className="space-y-3">
              {messages.map((m, i) => (
                <div
                  key={i}
                  className={`rounded-xl px-3 py-2 text-sm ${
                    m.role === "user" ? "ml-8 bg-gold/15" : "mr-4 bg-muted"
                  }`}
                >
                  {m.text}
                </div>
      ...
      <Button type="button" className="rounded-full shadow-lg" onClick={() => setOpen((o) => !o)}>
