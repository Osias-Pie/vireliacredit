-- Virelia Crédit — runtime hardening after local integration tests.
-- Idempotent repair migration: safe to apply after earlier Virelia migrations.
...
-- Legacy columns from the original grant-oriented schema must never block the new five-step form.
ALTER TABLE public.applications
...
BEGIN
  ...
  INSERT INTO public.applications (
    ...
    NULLIF(p->>'nationality',''), p->>'address', p->>'phone', p->>'email',
    NULLIF(p->>'employment_status',''), COALESCE(p->'employment_details', '{}'::jsonb),
    NULLIF(p->>'income','')::numeric, NULLIF(p->>'other_income','')::numeric,
    NULLIF(p->>'monthly_charges','')::numeric, p->>'program', (p->>'amount')::numeric,
    p->>'currency', NULLIF(p->>'duration_months','')::int, NULLIF(p->>'purpose',''),
    NULLIF(p->>'processing_speed',''), NULLIF(p->>'processing_fee','')::numeric,
    COALESCE(p->'documents', '[]'::jsonb), NULLIF(p->>'language',''),
    NULLIF(p->>'purpose',''), NULLIF(p->>'contract_path','')
  )
  ...
  INSERT INTO public.application_bank_details (
    ...
    new_id, p->>'bank_name', p->>'account_holder_name', p->>'iban_account_number', NULLIF(p->>'swift_bic','')
  )
  ON CONFLICT (application_id) DO UPDATE SET
    bank_name = EXCLUDED.bank_name,
    account_holder_name = EXCLUDED.account_holder_name,
    iban_account_number = EXCLUDED.iban_account_number,
...
DECLARE
  ...
  hist jsonb;
BEGIN
  IF p_reference IS NULL OR length(trim(p_reference)) < 8 THEN RETURN NULL; END IF;
